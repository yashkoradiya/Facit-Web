...
import EvaluatePricesFooter from '../../components/EvaluatePricesFooter';
...
export default function EvaluateCharterFlightPrices({ allRows, onClose, onPublished, showModal, currency }) {
  ...
  useEffect(() => {
    const data = allRows.map(flightSeries => {
      return {
        ...
        lastEvaluated: flightSeries.lastEvaluated,
  ...
  const handleEvaluatePrices = () => {
    ...
    Promise.all(publishInputModel.map(input => api.EvaluatePrices(input))).then(() => {
      setPublishing(false);
      setSaveSuccess(true);
  ...
  return (
    <ModalBase show={showModal} onRequestClose={onClose} width={'950px'} title={'Review and Evaluate'}>
      <Flexbox direction="column" width="100%" alignItems="flex-start">
        ...
        >
          ...
          <StickyResponsiveTableHeader>Last Evaluated</StickyResponsiveTableHeader>
        ...
        <EvaluatePricesFooter
          ...
          errors={false}
          onEvaluatePrices={handleEvaluatePrices}
          onClose={onClose}
