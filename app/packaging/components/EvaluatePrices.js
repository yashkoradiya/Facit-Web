...
class EvaluatePrices extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleEvaluatePrices = () => {
    ...
    const promises = jobData.map(x => {
      return this.props.onEvaluatePrices({ id: x.id, sourceMarketId: x.sourceMarketId, productType: x.productType });
    });
    ...
    return (
      <Flexbox direction="column" width="100%" alignItems="flex-start">
        <Gridbox
          ...
          columnDefinition={'35px 100px auto auto 65px 120px auto'}
          border
        >
          <StickyResponsiveTableHeader>
              ...
              return (
                <React.Fragment key={x[0].value}>
                  ...
                  {x.slice(1).map((cell, innerIndex) => {
                    return (
                      <ResponsiveTableCell key={`${index}-${innerIndex}`} hasTooltip={cell.hasTooltip}>
                        {cell.value}
                        {cell.hasTooltip && <PopoutToolTip>{cell.tooltipData}</PopoutToolTip>}
                      </ResponsiveTableCell>
                    );
                  })}
                </React.Fragment>
...
export default connect(mapStateToProps, mapDispatchToProps)(EvaluatePrices);
